...
个人消费记录 OpenClaw Skill。支持自然语言记账、自动分类、月度统计、关键词搜索、支出排行、预算提醒和精美 HTML 报表生成。
...
- **月度总览** — 总支出、预算环形进度、日均支出、消费 TOP5 排行
- **支出分布** — SVG 交互饼图，hover/点击查看详情
- **分类详情** — 各分类金额、占比、进度条
- **每日趋势** — SVG 柱状图 + 日均参考线
- **消费明细** — 支持搜索+分类+日期交叉筛选，联动饼图/分类卡片/趋势图
- **环比对比** — 与上月各分类涨跌对比
- **月份导航** — 顶部箭头快速切换月份
